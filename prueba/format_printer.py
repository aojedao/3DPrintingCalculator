...
class Ui_Dialog2(object):
    ...
    def save_dates(self):
        try:
            self.data={"diametro":float(self.diameter_printer.text()),"costo":float(self.cost_printer.text()),"tiempo_depreciacion":float(self.depreciation_time.text()),
                       "costo_servicio":float(self.service_cost.text()),"consumo_energetico":float(self.energy_consumption.text())}
        except ValueError:

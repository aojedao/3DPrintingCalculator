...
class cotizar(opciones):
    ...
    def savess(self):
        ...
        if ver>=0 :
            ...
            dta[ver][1][32].insertar(self.material_consumido())
            dta[ver][1][33].insertar(self.cost_filamento())
            dta[ver][1][34].insertar(self.cost_electricidad())
            dta[ver][1][35].insertar(self.depreciacion_impresora())
            dta[ver][1][36].insertar(self.preparacion())
            dta[ver][1][37].insertar(self.post_proces())
            dta[ver][1][38].insertar(self.subtotal(self.lists))
            dta[ver][1][39].insertar(self.incluye_fallos())
            dta[ver][1][40].insertar(self.precio_sug())
            ...
            matecon.insertar(self.material_consumido())
            new.append(matecon)
            filacos = list_array(x)
            filacos.insertar(self.cost_filamento())
            new.append(filacos)
            elecos = list_array(x)
            elecos.insertar(self.cost_electricidad())
            new.append(elecos)
            depres = list_array(x)
            depres.insertar(self.depreciacion_impresora())
            new.append(depres)
            prepa = list_array(x)
            prepa.insertar(self.preparacion())
            new.append(prepa)
            postpro = list_array(x)
            postpro.insertar(self.post_proces())
            new.append(postpro)
            sub = list_array(x)
            sub.insertar(self.subtotal(self.lists))
            new.append(sub)
            infail = list_array(x)
            infail.insertar(self.incluye_fallos())
            new.append(infail)
            sug = list_array(x)
            sug.insertar(self.precio_sug())
            new.append(sug)
    ...
    def export(self):
        ...
        grafica.insertar(self.material_consumido())
        grafica.insertar(self.cost_filamento())
        grafica.insertar(self.cost_electricidad())
        grafica.insertar(self.depreciacion_impresora())
        grafica.insertar(self.preparacion())
        grafica.insertar(self.post_proces())
        grafica.insertar(self.subtotal(self.lists))
        grafica.insertar(self.incluye_fallos())
        grafica.insertar(self.precio_sug())
...
class solicitud(opciones):
    ...
    def load_names(self,data):
        ...
        if len(data)>0:
            ...
            return names
        return []
    def return_data(self,name,data):
